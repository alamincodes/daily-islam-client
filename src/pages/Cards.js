...
const Cards = () => {
  const cards = [
    ...
    { name: "যিক্‌র", img: Tasbih, linkPage: "jikir" },
  ...
  return (
    <div>
      ...
      <div className=" flex justify-between items-center shadow-[0_3px_10px_rgb(0,0,0,0.1)] md:py-3 py-5 ">
       ...
       <h2 className=" md:text-4xl sm:text-[15px] font-semibold md:tracking-[0.5rem] sm:tracking-[2px]">{currentTime}</h2>
       </div>
        <button className="md:mx-10 sm:mx-3 md:text-[20px] sm:text-[15px] bg-black text-white rounded-lg py-3 md:px-6 sm:px-4  hover:scale-105 duration-100 ">নামাজ এর সময় দেখুন</button>
      </div>
      {/* ---------cards-------- */}
          ...
          <Link to={card.linkPage}>
            <div key={card.img}>
              <div className="flex  lg:flex-row md:flex-row sm:flex-col justify-center items-center shadow-[0_3px_10px_rgb(0,0,0,0.1)] py-5 px-2 transform hover:scale-105 duration-500">
                {" "}
                <h2 className="lg:text-3xl md:text-2xl sm:text-[16px] ">
                  {card.name}
                </h2>
